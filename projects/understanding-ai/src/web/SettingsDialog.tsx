...
const sections = ["Model", "Skills", "Connections", "Runtime"] as const;
export function SettingsDialog({ request, settings, setSettings, models, presets, onModels, onEvents, onClose }: Props) {
  ...
  function connection(selection: McpSelection, preset?: McpPreset) {
    ...
    return <article className="connectionCard" key={selection.id}>
      ...
      {result && <div className="discovered" role="status">{result.error || result.tools.map((tool) => <div key={tool.name}><code>{tool.name}</code><span>{tool.description}</span></div>)}</div>}
    </article>;
  }
    ...
    <div className="settingsShell">
      ...
      <div className="settingsLayout">
        ...
        <section className="settingsContent" aria-label={`${section} settings`}>
          ...
          {section === "Connections" && <><div className="sectionHeading"><div><h3>MCP connections</h3><p className="sectionIntro">Discover tools first, then enable a connection for the next run.</p></div><button className="primary" onClick={() => update({ id: `custom-${crypto.randomUUID().slice(0, 8)}`, enabled: false, url: "" })}>+ Add MCP</button></div>
          ...
          {section === "Runtime" && <><h3>Execution boundaries</h3><p className="sectionIntro">The harness manages context, executes tool requests and decides when to stop.</p><label className="settingField">Maximum model/tool rounds<input type="number" min={1} max={16} value={settings.maxRounds} onChange={(e) => setSettings((s) => ({ ...s, maxRounds: Math.max(1, Math.min(16, Number(e.target.value) || 1)) }))} /></label><div className="infoBox">Documents are parsed, chunked and embedded locally with MiniLM. Vectors live in a local JSON index under data/kb. Model assets may download on first use. Import content from the document library; follow each stage in Knowledge.</div></>}
        </section>
      </div>
